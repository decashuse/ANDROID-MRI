...
void StackVisitor::ValidateFrame() const {
  if (kIsDebugBuild) 
    ...
    if (!method->IsRuntimeMethod()) 
      ...
      CHECK_EQ(declaring_class->GetClass(), declaring_class->GetClass()->GetClass()) << declaring_class;
    } 
		else 
		{
      CHECK(declaring_class == nullptr);
    ...
    if (cur_quick_frame_ != nullptr) 
      ...
      if (method->IsNative()) 
        ...
        if (UNLIKELY(current_fragment->GetTopQuickFrameTag())) 
          ...
          if (!class_linker->IsQuickGenericJniStub(existing_entry_point) && !class_linker->IsQuickResolutionStub(existing_entry_point) &&
      ...
      while (method != nullptr) 
        ...
        if ((walk_kind_ == StackWalkKind::kIncludeInlinedFrames)
          ...
          if (stack_map->IsValid() && stack_map->HasInlineInfo()) 
            ...
            for (current_inline_frames_ = code_info->GetInlineInfosOf(*stack_map); !current_inline_frames_.empty(); current_inline_frames_.pop_back()) 
						{
