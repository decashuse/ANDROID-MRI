...
void Thread::CreateNativeThread(JNIEnv* env, jobject java_peer, size_t stack_size, bool is_daemon) {
  ...
  if (VLOG_IS_ON(threads)) 
    ...
    VLOG(threads) << "Creating native thread for " << thread_name;
		//decash
    //	DECASH_ART ( android::base::StringPrintf("NativeThread(%d) : %s", getuid(), thread_name.c_str()) );
		//////
...
//decash/////////////////
void Thread::DecashDump(std::ostream& os, bool dump_native_stack, BacktraceMap* backtrace_map,
                  bool force_dump_stack) const {
  DecashDumpStack(os, dump_native_stack, backtrace_map, force_dump_stack);
}
///////////////////////
...
struct StackDumpVisitor : public MonitorObjectsStackVisitor {
  ...
  VisitMethodResult StartMethod(ArtMethod* m, size_t frame_nr ATTRIBUTE_UNUSED) override REQUIRES_SHARED(Locks::mutator_lock_) 
	{
    m = m->GetInterfaceMethodIfProxy(kRuntimePointerSize);
    ObjPtr<mirror::DexCache> dex_cache = m->GetDexCache();
    int line_number = -1;
    if (dex_cache != nullptr) 
		{  // be tolerant of bad input
...
void Thread::DumpJavaStack(std::ostream& os, bool check_suspended, bool dump_locks) const {
  ...
  StackDumpVisitor dumper(os, const_cast<Thread*>(this), context.get(), !tls32_.throwing_OutOfMemoryError, check_suspended, dump_locks);
  dumper.WalkStack();
}
...
//decash//////////////////////////
void Thread::DecashDumpStack(std::ostream& os,
				bool dump_native_stack,
				BacktraceMap* backtrace_map,
				bool force_dump_stack) const 
{
		...
		if (safe_to_dump || force_dump_stack) 
		{
				// decash if you want native dump
				// dump_native_stack = true;
				// //if (dump_native_stack && (dump_for_abort || force_dump_stack || ShouldShowNativeStack(this))) 
				if (dump_native_stack && (dump_for_abort || force_dump_stack || ShouldShowNativeStack(this))) 
				{    
						ArtMethod* method = GetCurrentMethod(nullptr, !force_dump_stack, !(dump_for_abort || force_dump_stack));
						DumpNativeStack(os, GetTid(), backtrace_map, "  native: ", method);
				}    
				DumpJavaStack(os, !force_dump_stack, !force_dump_stack);
		} 
		else 
		{
				os << "Not able to dump stack of thread that isn't suspended";
