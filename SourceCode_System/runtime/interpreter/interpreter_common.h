...
static ALWAYS_INLINE bool DoInvoke(Thread* self,
  ...
  if (is_mterp && !is_range && called_method->IsIntrinsic()) 
	{
    if (MterpHandleIntrinsic(&shadow_frame, called_method, inst, inst_data, shadow_frame.GetResultRegister())) 
		{
  ...
  if (use_fast_path) {
    ...
    ShadowFrameAllocaUniquePtr shadow_frame_unique_ptr = CREATE_SHADOW_FRAME(num_regs, &shadow_frame, called_method, /* dex pc */ 0);
    ...
    if (is_range) 
      ...
      for (size_t i = 0, dst = first_dest_reg; i < number_of_inputs; ++i, ++dst) 
			{
			  //InstructionHandler::DispByRegId(arg[i]);
    ...
    for (uint32_t i = 0; i < shadow_frame.NumberOfVRegs(); ++i) 
      ...
      if (ref_value != nullptr) 
        ...
        if (ref_value->GetClass()->IsStringClass() && !ref_value->AsString()->IsValueNull()) 
          oss << "/string \"" << ref_value->AsString()->ToModifiedUtf8() << "\"";
				////////////////
...
//decash
static inline void TraceExecution1(const ShadowFrame& shadow_frame, const Instruction* inst, const uint32_t dex_pc)
    REQUIRES_SHARED(Locks::mutator_lock_) {

    std::ostringstream oss;
    oss  << shadow_frame.GetMethod()->PrettyMethod(); 
		oss  << android::base::StringPrintf(" 0x%04x: ", dex_pc) << inst->DumpString(shadow_frame.GetMethod()->GetDexFile()) << "\n\n"; 		
		DECASH_ART( oss.str()  );
}
//decash
