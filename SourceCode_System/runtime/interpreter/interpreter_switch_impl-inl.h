...
#define HANDLER_ATTRIBUTES ALWAYS_INLINE FLATTEN WARN_UNUSED REQUIRES_SHARED(Locks::mutator_lock_)
  ...
  HANDLER_ATTRIBUTES bool CheckForceReturn() {
	...
	//decash ///////////////////////////////////
  std::string GetPrevMethodName() REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					for (const ManagedStack* current_fragment = self_->GetManagedStack(); current_fragment != nullptr; current_fragment = current_fragment->GetLink())
					{
									ShadowFrame* pTempShadowFrame = current_fragment->GetTopShadowFrame();
									if(pTempShadowFrame == &shadow_frame_) pTempShadowFrame = pTempShadowFrame->GetLink(); 
									while(1)
									{
													if(pTempShadowFrame) 
													{
																		return pTempShadowFrame->GetMethod()->PrettyMethod();
													}
													else 
																	break;
									}
  ...
  void PrintAllPrevMethod() REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					std::ostringstream ossGraphData;
					ShadowFrame* pCallerShadowFrame = nullptr;
					ShadowFrame* pCalleeShadowFrame = nullptr;

					for (const ManagedStack* current_fragment = self_->GetManagedStack(); current_fragment != nullptr; current_fragment = current_fragment->GetLink())
					{
									ShadowFrame* pCurShadowFrame = current_fragment->GetTopShadowFrame();
									if( !pCalleeShadowFrame ) 
									{
										pCalleeShadowFrame = pCurShadowFrame;
										pCurShadowFrame = pCurShadowFrame->GetLink();
									}
									while(1)
									{
													pCallerShadowFrame = pCurShadowFrame;

													if(pCallerShadowFrame)
													{
																	ossGraphData << "\"" <<  pCallerShadowFrame->GetMethod()->PrettyMethod() << "\"" << " -> "  
																					<< "\"" <<  pCalleeShadowFrame->GetMethod()->PrettyMethod() << "\"" << "\n" ;    

																	pCalleeShadowFrame = pCallerShadowFrame;
																	pCurShadowFrame  = pCurShadowFrame->GetLink();
													}
													else 
																	break;
					...
					DECASH_ART( ossGraphData.str() );
	}

 	void DispByRegId(int32_t nRegId) REQUIRES_SHARED(Locks::mutator_lock_) 
	{
			if( is_target_uid() )
			{
					if( get_trace_mode() == 2 )
								DumpInstInfo(nRegId);
					else if( get_trace_mode() == 3 )
								DumpGraphInfo(nRegId);
					else if( get_trace_mode() == 5 )
								DumpStringInfo(nRegId);
						
			}
	}
	
  void DumpInstInfo(int32_t nRegId) REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					std::ostringstream ossInstInfo;
					
					ObjPtr<mirror::Object> ref_value = shadow_frame_.GetVRegReference(nRegId);
					//PrintObject(ref_value, nRegId);

					if (ref_value != nullptr)
					{
									// java.lang.String
									if (ref_value->GetClass()->IsStringClass() && !ref_value->AsString()->IsValueNull())
									{
													if( check_target_search_item(ref_value->AsString()->ToModifiedUtf8().c_str())) 
													{
																	std::string strCurMethodName = shadow_frame_.GetMethod()->PrettyMethod();
																	if( check_target_method(strCurMethodName.c_str()) )
																	{
																					ossInstInfo << strCurMethodName << "\n" 
																					            << android::base::StringPrintf("0x%x: ", dex_pc_) << inst_->DumpStringDecash(shadow_frame_.GetMethod()->GetDexFile()) 
																											<< "[v" << nRegId << "/(str)" << ref_value->AsString()->ToModifiedUtf8() << "]" << "\n\n"; 
																					DECASH_ART( ossInstInfo.str() );
																	}
													}
									...
									// byte[]
									else if( ref_value->IsByteArray() )
									{
													ObjPtr<mirror::Array> array = ref_value->AsArray();
													...
													if( check_target_search_item(strByteArray.c_str()) )
													{
																	...
																	{				
																					ossInstInfo << strCurMethodName << "\n" 
																					            << android::base::StringPrintf("0x%x: ", dex_pc_) << inst_->DumpStringDecash(shadow_frame_.GetMethod()->GetDexFile()) 
																											<< "[v" << nRegId << "/(byte)" << strByteArray.c_str() << "]" << "\n\n"; 
																					DECASH_ART( ossInstInfo.str() );
																	}
													}
									...
									else if( ref_value->IsCharArray() )
									{
													...
													for(int a = 0; a < nSize; a++)
																	temp[a] = *(((char*)(char_array->GetData()))+a*2);
													...
													if( check_target_search_item(strCharArray.c_str()) )
													{
																	...
																	if( check_target_method(strCurMethodName.c_str()) )
																	{
																					ossInstInfo << strCurMethodName << "\n" 
																					            << android::base::StringPrintf("0x%x: ", dex_pc_) << inst_->DumpStringDecash(shadow_frame_.GetMethod()->GetDexFile()) 
																											<< "[v" << nRegId << "/(char)" << strCharArray.c_str() << "]" << "\n\n"; 
																					DECASH_ART( ossInstInfo.str() );
																	}
													}
	...
	void PrintObject(ObjPtr<mirror::Object> ref_value, int nRegId ) REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					std::ostringstream ossInstInfo;
					if (ref_value != nullptr)
					{
									...
									//  class type
									//else if( ref_value->GetClass()->IsObjectClass() ) 
									else if( !ref_value->GetClass()->IsPrimitive() ) 
									{
													for (ArtField& field : ref_value->GetClass()->GetIFields())
													{
																	if( strncmp(field.GetTypeDescriptor(), "L", 1) == 0)
																	{
																					ObjPtr<mirror::Object> objx = field.GetObj(ref_value);
																					ossInstInfo << "START CHECK : " << field.PrettyField() << "\n";
																					DECASH_ART( ossInstInfo.str() );
																					PrintObject(objx, nRegId);
																	}

													}
									}
									DECASH_ART( "END CHECK \n\n" );
				}


	}

  void DumpGraphInfo(int32_t nRegId) REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					std::ostringstream ossGraphData;

					std::string strNexMethodName;  
					std::string strPreMethodName; 
					std::string strCurMethodName;
					...
					if (ref_value != nullptr)
					{
									...
									if (ref_value->GetClass()->IsStringClass() && !ref_value->AsString()->IsValueNull())
									{
													if( check_target_search_item(ref_value->AsString()->ToModifiedUtf8().c_str())) 
													{
																	strCurMethodName = shadow_frame_.GetMethod()->PrettyMethod();
																	if( check_target_method(strCurMethodName.c_str()) ) 
																	{		
																			ossGraphData << "\"" <<  strCurMethodName << "\"" << " [fillcolor=green style=filled]" << "\n" ;  

																			switch( (uint8_t) (inst_->Opcode()) )
																			{
																					case 0x6E: case 0x6F: case 0x70: case 0x71: case 0x72:
																							strNexMethodName = shadow_frame_.GetMethod()->GetDexFile()->PrettyMethod(inst_->VRegB_35c()); 
																							//ossGraphData << "\"" <<  strCurMethodName << "\"" << " -> " << "\"" << strNexMethodName << "\"" << " [color=green]"<< "\n"   
																							ossGraphData << "\"" <<  strCurMethodName << "\"" << " -> " << "\"" << strNexMethodName << "\"" << "\n"   
																							             << "\"" <<  strNexMethodName << "\"" << " [fillcolor=green style=filled]" << "\n" ;  
																							break;
																					case 0x74: case 0x75: case 0x76: case 0x77: case 0x78:
																							strNexMethodName = shadow_frame_.GetMethod()->GetDexFile()->PrettyMethod(inst_->VRegB_3rc()); 
																							//ossGraphData << "\"" <<  strCurMethodName << "\"" << " -> " << "\"" << strNexMethodName << "\"" << " [color=green]"<< "\n"   
																							ossGraphData << "\"" <<  strCurMethodName << "\"" << " -> " << "\"" << strNexMethodName << "\"" << "\n"   
																							             << "\"" <<  strNexMethodName << "\"" << " [fillcolor=green style=filled]" << "\n" ;  
																							break;
									...
									/*
									// is object
									else// if( ref_value->IsObjectClass() ) 
									{
											for (ArtField& field : ref_value->GetClass()->GetIFields())
											{
															// java.lang.String
															if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
															{
																			...
																			if(objx != nullptr)
																			{
																							if( check_target_search_item( objx->AsString()->ToModifiedUtf8().c_str()) )

																							/////////////
																							//strCurMethodName = shadow_frame_.GetMethod()->PrettyMethod();
																							//if( check_target_method(strCurMethodName.c_str()) ) 
																							{		
																											ossGraphData << "\"" <<  field.PrettyField() << "\"" << " [FUCKING]" << "\n" ;  
															...
															else if( strcmp(field.GetTypeDescriptor(), "[B") == 0 )
															{
																			ObjPtr<mirror::Object> objx = field.GetObj(ref_value);
																			if(objx != nullptr)
																			{

																							ObjPtr<mirror::Array> array = objx->AsArray();
																							ObjPtr<mirror::ByteArray> byte_array = array->AsByteArray();
  ...
  void DumpStringInfo(int32_t nRegId) REQUIRES_SHARED(Locks::mutator_lock_) 
	{
					std::ostringstream ossInstInfo;
					...
					/*
									...
									if (ref_value->GetClass()->IsStringClass() && !ref_value->AsString()->IsValueNull())
									{
													...
													if( check_target_method(strCurMethodName.c_str()) )
													{
																	//ossInstInfo <<  "(str)" << ref_value->AsString()->ToModifiedUtf8() << "\n"; 
																	//DECASH_ART( ossInstInfo.str() );
																	DECASH_ART(  "(str)" + ref_value->AsString()->ToModifiedUtf8() + "\n");
													}
									...
									else if( ref_value->IsByteArray() )
									{
													...
													if( check_target_method(strCurMethodName.c_str()) )
													{				
																	//ossInstInfo << "(byte)" << strByteArray.c_str() << "\n"; 
																	//DECASH_ART( ossInstInfo.str() );
																	DECASH_ART(  "(byte)" + strByteArray + "\n");
													}
									...
									else if( ref_value->IsCharArray() )
									{
													...
													if( check_target_method(strCurMethodName.c_str()) )
													{
																	//ossInstInfo <<  "(char)" << strCharArray.c_str() << "\n"; 
																	//DECASH_ART( ossInstInfo.str() );
																	DECASH_ART(  "(char)" + strCharArray + "\n");
													}
						...
						*/
							...
							if( ref_value->IsByteArray() )
							{
									ObjPtr<mirror::Array> array = ref_value->AsArray();
									ObjPtr<mirror::ByteArray> byte_array = array->AsByteArray();

									// key size 128 or 256
									int nSizeOfByteArray = byte_array->GetLength();
									if( nSizeOfByteArray == 16 || nSizeOfByteArray == 32 )
									{
											char* temp = new char[byte_array->GetLength()+1];
											memset(temp, 0x00, byte_array->GetLength()+1);
											memcpy(temp, byte_array->GetData(), byte_array->GetLength());
...
#pragma clang diagnostic pop
  ...
  HANDLER_ATTRIBUTES bool HandleInvoke() {
	//decash
	if( is_target_uid() && (get_trace_mode() == 2 || get_trace_mode() == 3 || get_trace_mode() == 5) )
	{
					const uint16_t number_of_inputs = (is_range) ? inst_->VRegA_3rc(inst_data_) : inst_->VRegA_35c(inst_data_);
					if(is_range)
					{
									size_t src = (is_range) ? inst_->VRegC_3rc() : inst_->VRegC_35c();
									for (size_t i = 0; i < number_of_inputs; ++i, ++src) 
									{   
													DispByRegId(src);
									}   
					}
					else
					{
									...
									inst_->GetVarArgs(arg, inst_data_);
  ...
  HANDLER_ATTRIBUTES bool FILL_ARRAY_DATA() {
    ...
    const Instruction::ArrayDataPayload* payload = reinterpret_cast<const Instruction::ArrayDataPayload*>(payload_addr);
    ObjPtr<mirror::Object> obj = GetVRegReference(A());
    ...
    if (transaction_active) 
      ...
      RecordArrayElementsInTransaction(obj->AsArray(), payload->element_count);
		...
		if(is_target_uid())
		{
				...
				else if( is_display_instruction() )
				{
						if( obj->IsByteArray() )
						{
								...
								DECASH_ART(shadow_frame_.GetMethod()->PrettyMethod() + " : " +   obj->PrettyTypeOf() + " : " +strByteArray);
						...
						else if( obj->IsCharArray())
						{
										...
										std::string strCharArray(temp);
										DECASH_ART(shadow_frame_.GetMethod()->PrettyMethod() + " : " +   obj->PrettyTypeOf() + " : " + strCharArray);
										delete[] temp;
						}
			}
  ...
  HANDLER_ATTRIBUTES bool IGET_OBJECT() {
    //return HandleGet<InstanceObjectRead, Primitive::kPrimNot>();
		//decash
    bool result = HandleGet<InstanceObjectRead, Primitive::kPrimNot>();
		DispByRegId(A());
		return result;
  }
  ...
  HANDLER_ATTRIBUTES bool SGET_OBJECT() {
    //return HandleGet<StaticObjectRead, Primitive::kPrimNot>();
		//decash
    bool result =  HandleGet<StaticObjectRead, Primitive::kPrimNot>();
		DispByRegId(A());
		return result;
  }
...
void ExecuteSwitchImplCpp(SwitchImplContext* ctx) {
  ...
  while (true) {
    ...
    if (InstructionHandler<do_access_check, transaction_active, Instruction::kInvalidFormat>(ctx, instrumentation, self, shadow_frame, dex_pc, inst, inst_data, next, exit).Preamble()) 	 {
...
template<bool do_access_check, bool transaction_active>
void ExecuteDecashSwitchImplCpp(SwitchImplContext* ctx) {
  Thread* self = ctx->self;
  const CodeItemDataAccessor& accessor = ctx->accessor;
  ShadowFrame& shadow_frame = ctx->shadow_frame;
  self->VerifyStack();
...
// FOR TraceExecution1 (Instruction Dump Only) for decompiler using CHATGPT
template<bool do_access_check, bool transaction_active>
void ExecuteDecashSwitchImplCpp1(SwitchImplContext* ctx) {
  Thread* self = ctx->self;
  const CodeItemDataAccessor& accessor = ctx->accessor;
  ShadowFrame& shadow_frame = ctx->shadow_frame;
  self->VerifyStack();
  ...
  while (true) {
    ...
    shadow_frame.SetDexPC(dex_pc);
		...
		//decash
		TraceExecution1(shadow_frame, inst, dex_pc);
