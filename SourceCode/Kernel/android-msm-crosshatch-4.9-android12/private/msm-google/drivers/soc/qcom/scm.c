				...
				size_t scm_buf_length)
{
	...
	//DECASH_1("SMC CALL : SVC_ID = %X, CMD_ID : %X", svc_id, cmd_id);
...
static int __scm_call2(u32 fn_id, struct scm_desc *desc, bool retry)
{
    ...
    //DECASH_1("scm_call: func id %#llx, ret: %d, syscall returns: %#llx, %#llx, %#llx\n", x0, ret, desc->ret[0], desc->ret[1], desc->ret[2]);
	//if( SCM_SVC_ID(fn_id) != 0xd)
    // 	DECASH_1("scm_call: func id %#llx", SCM_SVC_ID(fn_id));
