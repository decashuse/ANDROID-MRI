...
JValue InvokeProxyInvocationHandler(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if (LIKELY(!soa.Self()->IsExceptionPending())) 
  {
	...
	{
      ArtMethod* interface_method = soa.Decode<mirror::Method>(interface_method_jobj)->GetArtMethod();
    ...
    if (exception->IsCheckedException()) {
      bool declares_exception = false;
      {
        ...
        // the slice.
        int throws_index = (reinterpret_cast<uintptr_t>(proxy_method) - reinterpret_cast<uintptr_t>(&virtual_methods[0])) / method_size;
        CHECK_LT(throws_index, static_cast<int>(num_virtuals));
        ObjPtr<mirror::ObjectArray<mirror::Class>> declared_exceptions = proxy_class->GetProxyThrows()->Get(throws_index);
      ...
      if (!declares_exception) {
        soa.Self()->ThrowNewWrappedException("Ljava/lang/reflect/UndeclaredThrowableException;", nullptr);
      }
    }
    return zero;
  }
