  ...
  if (LIKELY(Runtime::Current()->IsStarted())) 
    ...
    if (use_interpreter_entrypoint) 
      ...
      interpreter::ArtInterpreterToInterpreterBridge(self, accessor, callee_frame, result);
		...
		{
      interpreter::ArtInterpreterToCompiledCodeBridge(self, caller_method, callee_frame, first_dest_reg, result);
    }
  } 
	else 
	{
  ...
  switch (field_type) {
    ...
    case Primitive::kPrimNot: {
      ...
      field->SetObj<transaction_active>(obj, reg);
			...
			if( is_target_uid() )
			{
					if( obj->IsByteArray() || obj->IsCharArray())
					{
							DECASH_ART(shadow_frame.GetMethod()->PrettyMethod() + " : " +
							obj->PrettyTypeOf() + " : " +
							std::string((const char*)payload->data, payload->element_count));
					}
					else 
					//if( reg->IsString() )
					if(obj != nullptr)
