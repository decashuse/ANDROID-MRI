...
//decash
void DisplayInputSearchItems(Thread* self, const CodeItemDataAccessor& accessor, ShadowFrame& shadow_frame, ArtMethod* MyMethod,  const char* shorty)
REQUIRES_SHARED(Locks::mutator_lock_) 
{
	  uint16_t arg_offset = accessor.RegistersSize() - accessor.InsSize();
	  size_t   arg_pos    = arg_offset;
	  size_t   arg_count  = shadow_frame.NumberOfVRegs() ;
	  if (!MyMethod->IsStatic()) arg_pos++;

		  for (size_t shorty_pos = 0; arg_pos < arg_count; ++shorty_pos, ++arg_pos)
		  {
			  switch (shorty[shorty_pos + 1])
			  {
				  case 'L':
				  {
					  ObjPtr<mirror::Object> obj= shadow_frame.GetVRegReference(arg_pos);
					  if(obj == nullptr) break;

						// string
					  if( obj->IsString() )
						{
										if( check_target_search_item(obj->AsString()->ToModifiedUtf8().c_str()) )
										{ 
														DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
														DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] [CLASS.STRING] => " + obj->AsString()->ToModifiedUtf8() );

														if( is_display_stack_trace() ) self->DecashDump(DECASH_ART(""), false);

										}
						}
					  
						// byte[]
						else if( obj->IsByteArray() )
						{
										ObjPtr<mirror::Array> array = obj->AsArray();
										ObjPtr<mirror::ByteArray> byte_array = array->AsByteArray();

										char* temp = new char[byte_array->GetLength()+1];
										memset(temp, 0x00, byte_array->GetLength()+1);
										memcpy(temp, byte_array->GetData(), byte_array->GetLength());
										std::string strByteArray(temp);
										delete[] temp;

										if( check_target_search_item(strByteArray.c_str()) )
										{ 
														DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
														DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] [BYTE[]] => " + strByteArray );
						...
						// char[]
						else if( obj->IsCharArray() )
						{
										ObjPtr<mirror::Array> array = obj->AsArray();
										ObjPtr<mirror::CharArray> char_array = array->AsCharArray();

										int nSize = char_array->GetLength();
										char* temp  = new char[nSize];

										for(int i = 0; i < nSize; i++) 
														temp[i] = *(((char*)(char_array->GetData()))+i*2);
										std::string strCharArray(temp);
										delete[] temp;

										if( check_target_search_item(strCharArray.c_str()) )
										{ 
														DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
														DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] [CHAR[]] => " + strCharArray );
...
//decash
void DisplayInputValue(Thread* self, const CodeItemDataAccessor& accessor, ShadowFrame& shadow_frame, ArtMethod* MyMethod,  const char* shorty)
REQUIRES_SHARED(Locks::mutator_lock_) 
{
	  uint16_t arg_offset = accessor.RegistersSize() - accessor.InsSize();
	  size_t   arg_pos    = arg_offset;
	  ...
	  if (!MyMethod->IsStatic()) arg_pos++;
		  ...
		  // STACK TRACE
		  if( is_display_stack_trace() )
			  self->DecashDump(DECASH_ART(""), false);
		  //////////////////////


		  // DUMP CODE
		  if( is_display_code_dump() )
		  {
			  if (accessor.HasCodeItem()) 
			  {
				  for (const DexInstructionPcPair& inst : accessor) 
				  {
						  ...
						  //DECASH_ART( android::base::StringPrintf("0x%04x: ", inst.DexPc()) << inst->DumpHexLE(5)  );
						  DECASH_ART( android::base::StringPrintf("   | %s", inst->DumpString(MyMethod->GetDexFile()).c_str()) );
				  }
			  }
		  }
		  /////////////////////
		  ...
		  if( is_display_args() )
			  for (size_t shorty_pos = 0; arg_pos < arg_count; ++shorty_pos, ++arg_pos)
			  {
				  switch (shorty[shorty_pos + 1])
				  {
					  case 'L':
					  {
						  ...
						  if ( obj->IsString())
							...
							else if( obj->IsCharArray() )
							{
											...
											DECASH_ART( "  └[" + std::to_string(shorty_pos+1) + "] " + "[CHAR[]]" + " => " + std::string(temp) );
											delete[] temp;
							}
							else
							{
							  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] [CLASS] => " + obj->PrettyTypeOf() );

								/*
								// DISP Fields(Instance Fields) Info
								for (ArtField& field : obj->GetClass()->GetIFields()) 
								{                                  
												if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
												{
																ObjPtr<mirror::String> objx = obj->GetFieldObject<mirror::String>(field.GetOffset());
																if(objx != nullptr)
																	DECASH_ART("     └[" + field.PrettyField() + " => [IFOCK]" + objx->ToModifiedUtf8() );
												}
								}
								*/
								...
								for (ArtField& field : obj->GetClass()->GetIFields()) 
								{
												if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
												{
																...
																ObjPtr<mirror::Object> objx = field.GetObj(obj);
																if(objx != nullptr)
																				DECASH_ART("     └[" + field.PrettyField() + " => [IFOCK]" + objx->AsString()->ToModifiedUtf8() );
												}

								}
								/*
								for (ArtField& field : obj->GetClass()->GetSFields()) 
								{
												...
												if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
												{
																...
																ObjPtr<mirror::Object> objx = field.GetObj(obj);
																if(objx != nullptr)
																				DECASH_ART("     └[" + field.PrettyField() + " => [SFOCK]"  );
								...
								*/
								...
								/*
								// DISP Fields(Static Fields) Info
								for (ArtField& field : obj->GetClass()->GetSFields()) 
								{												
												if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
												{
																...
																DECASH_ART("     └[" + field.PrettyField() + " => [SFOCK]" );
																//if(objx != nullptr)
																//				DECASH_ART("     └[" + field.PrettyField() + " => [SFOCK]" + objx->ToModifiedUtf8() );
												}
					  ...
					  case 'C':
					  {
						  char ch = shadow_frame.GetVReg(arg_pos);
						  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[CHAR]"  + " => " + ch);
					  ...
					  {
						  char ch = shadow_frame.GetVReg(arg_pos);
						  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[BYTE]"  + " => " + ch);

						  break;
					  }
					  case 'I':
					  {
						  int num = shadow_frame.GetVReg(arg_pos);
						  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[INT]"  + " => " + std::to_string(num) );

						  break;
					  }
					  case 'Z':
					  {
						  bool num = shadow_frame.GetVReg(arg_pos);
						  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[BOOLEAN]" + " => " + (num ? "true" : "false") );
					  ...
					  case 'J': case 'D':
					  {
						  // shadow_frame.GetVRegLong(arg_pos);
						  // shadow_frame.GetVRegDouble(arg_pos);
						  // shadow_frame.GetVRegFloat(arg_pos);
						  arg_pos++;
						  break;
					  }
					  default:
					  {
						  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[FUCK] => fuck" );
						  break;
					  }
				  } // END SWITCH(check args type)
			  } // END FOR(loop PROCESS EACH ARGS)
...
//decash
void DisplayReturnSearchItems(Thread *self, ArtMethod* MyMethod, char chType, JValue result)
REQUIRES_SHARED(Locks::mutator_lock_) 
{
				if(chType == 'L' ) 
				{
								ObjPtr<mirror::Object> obj = result.GetL();
								if(obj != nullptr)
								{

												std::string temp1 = MyMethod->PrettyMethod();
												std::size_t pos = temp1.find(' ');
												std::string strRet = temp1.substr(0, pos);


												//if( obj->IsString() )
												if( strRet.compare("java.lang.String") == 0 )
												{
																if( check_target_search_item(obj->AsString()->ToModifiedUtf8().c_str()) )
																{										
																				DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
																				DECASH_ART("  └[R] [CLASS.STRING] => " + obj->AsString()->ToModifiedUtf8() );
												...
												//else if( obj->IsByteArray() )
												else if( strRet.compare("byte[]") == 0 )
												{
																ObjPtr<mirror::Array> array = obj->AsArray();
																ObjPtr<mirror::ByteArray> byte_array = array->AsByteArray();
																std::string strByteArray((char*)byte_array->GetData(), byte_array->GetLength());

																if( check_target_search_item(strByteArray.c_str()) )
																{
																				DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
																				DECASH_ART("  └[R] [BYTE[]] => " + strByteArray );
												...
												else if( strRet.compare("char[]") == 0 )
												{
																...
																{
																				DECASH_ART( "[B]" + MyMethod->PrettyMethod() );
																				DECASH_ART("  └[R] [CHAR[]] => " + strCharArray );
...
//decash
void DisplayReturnValue(char chType, JValue& result)
REQUIRES_SHARED(Locks::mutator_lock_) 
{
	if( is_display_args() )
	{    
			switch (chType) 
			{
				case 'L': 
				{
				  ObjPtr<mirror::Object> obj = result.GetL();
					if ( obj != nullptr)
					{
						//ObjPtr<mirror::Class> cls = obj->AsClass();
						//if( !cls->IsErroneous() )
						//DECASH_ART("  └[R] [CLASS] => " + obj->PrettyTypeOf() );
						...
						else if( obj->IsCharArray() )
						{
										...
										DECASH_ART("  └[R] [CHAR[]] => " + std::string(temp) );
										delete[] temp;
						}
						else
						{
				...
				{
					char ch = result.GetC();
					DECASH_ART(std::string("  └[R] [CHAR] => ") + ch); 
				...
				{
					int num = result.GetI();
					DECASH_ART(std::string("  └[R] [INT] => ") + std::to_string(num) );
				...
				case 'Z':
				{
					bool num = result.GetZ();
					DECASH_ART(std::string("  └[R] [BOOLEAN] => ") + (num ? "true" : "false") );
                            ...
                            bool interpret_one_instruction) REQUIRES_SHARED(Locks::mutator_lock_)
{
				if (Runtime::Current()->IsActiveTransaction()) 
				{
								...
								{
												return ExecuteSwitchImpl<false, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteSwitchImpl<true, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				} 
				else 
				{
								if (shadow_frame.GetMethod()->SkipAccessChecks()) 
								{
												return ExecuteSwitchImpl<false, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteSwitchImpl<true, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				}
}

static JValue ExecuteDecashSwitch(Thread* self,
                            const CodeItemDataAccessor& accessor,
                            ShadowFrame& shadow_frame,
                            JValue result_register,
                            bool interpret_one_instruction) REQUIRES_SHARED(Locks::mutator_lock_)
{
				if (Runtime::Current()->IsActiveTransaction()) 
				{
								if (shadow_frame.GetMethod()->SkipAccessChecks()) 
								{
												return ExecuteDecashSwitchImpl<false, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteDecashSwitchImpl<true, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				} 
				else 
				{
								if (shadow_frame.GetMethod()->SkipAccessChecks()) 
								{
												return ExecuteDecashSwitchImpl<false, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteDecashSwitchImpl<true, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				}
}

static JValue ExecuteDecashSwitch1(Thread* self,
                            const CodeItemDataAccessor& accessor,
                            ShadowFrame& shadow_frame,
                            JValue result_register,
                            bool interpret_one_instruction) REQUIRES_SHARED(Locks::mutator_lock_)
{
				if (Runtime::Current()->IsActiveTransaction()) 
				{
								if (shadow_frame.GetMethod()->SkipAccessChecks()) 
								{
												return ExecuteDecashSwitchImpl1<false, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteDecashSwitchImpl1<true, true>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				} 
				else 
				{
								if (shadow_frame.GetMethod()->SkipAccessChecks()) 
								{
												return ExecuteDecashSwitchImpl1<false, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								} 
								else 
								{
												return ExecuteDecashSwitchImpl1<true, false>(self, accessor, shadow_frame, result_register, interpret_one_instruction);
								}
				}
}
...
static inline JValue Execute(
  ...
  //decash//////////////////////////////////////
  ArtMethod *MyMethod = shadow_frame.GetMethod();
  uint32_t    shorty_len = 0; 
  const char* shorty     = MyMethod->GetShorty(&shorty_len);

  if( is_target_uid() /*&& (strncmp(GetIsoDate().c_str(), "2023", 4) == 0)*/)
  {
		  stay_in_interpreter = true;

		  /*
		  // it's temp code for Secure Screen bypass and it's for CanSecWest
		  if( strcmp((const char*)MyMethod->PrettyMethod().c_str(), "void android.view.Window.addFlags(int)") == 0 )
		  {
				  uint16_t arg_offset = accessor.RegistersSize() - accessor.InsSize();
				  size_t   arg_pos    = arg_offset;
				  if (!MyMethod->IsStatic()) arg_pos++;
				  ...
				  int num = shadow_frame.GetVReg(arg_pos);
				  if(num == 8192) shadow_frame.SetVReg(arg_pos, 0);
		  }
		  // END tempcode, CanSecWest
		  */
		  /* secure screen bypass
		  if( strcmp((const char*)MyMethod->PrettyMethod().c_str(), "void android.view.View.setFlags(int,int)") == 0 )
		  {
				  uint16_t arg_offset = accessor.RegistersSize() - accessor.InsSize();
				  size_t   arg_pos    = arg_offset;
				  if (!MyMethod->IsStatic()) arg_pos++;
				  ...
				  {
				  	num = num & ~0x2000;
				  	shadow_frame.SetVReg(arg_pos, num);
					DECASH_ART( "FUCK PATCH 1111 -----------------------~~~~!!!" );
				  }
					DECASH_ART( "FUCK PATCH 2222 -----------------------~~~~!!!" );
				  //
				  if(num & 8192) 
				  {
				  	shadow_frame.SetVReg(arg_pos, num & ~8192);
					DECASH_ART( "FUCK PATCH 222222222 -----------------------~~~~!!!" );
				  }
				  ///
		  ...
		  */
		  ...
		  if( get_trace_mode() == 4 )
		  {
				  if( is_search_item_exist() )
				  {
				  		  if( check_target_method( MyMethod->PrettyMethod().c_str() ) )
						  {
						  		DisplayInputSearchItems(self, accessor, shadow_frame, MyMethod, shorty);
						  }
				  }
				  else if( check_target_method( MyMethod->PrettyMethod().c_str() ) )
				  {
						  DisplayInputValue(self, accessor, shadow_frame, MyMethod, shorty);
				  }
		  }
  }
  /////////////////////////////////////////////////
  ...
  {
		  //decash
		  //return ExecuteSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
		  JValue temp; 
		  if( is_target_uid() /*&& (strncmp(GetIsoDate().c_str(), "2023", 4) == 0)*/) 
		  {
				  if( get_trace_mode() == 1 )
				  {
						  if( check_target_method(method->PrettyMethod().c_str()) )
								  temp = ExecuteDecashSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
						  else
								  temp = ExecuteSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
				  }

				  else if( get_trace_mode() == 6 )
				  {
						  if( check_target_method(method->PrettyMethod().c_str()) )
								  temp = ExecuteDecashSwitch1(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
						  else
								  temp = ExecuteSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
				  }

				  else if( get_trace_mode() == 4 )
				  {
						  temp = ExecuteSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);

						  if( is_search_item_exist() )
								  DisplayReturnSearchItems(self, MyMethod, shorty[0], temp);
						  else if( check_target_method( MyMethod->PrettyMethod().c_str() ) )
								  DisplayReturnValue(shorty[0], temp);
				  }
				  else
						  temp = ExecuteSwitch(self, accessor, shadow_frame, result_register, /*interpret_one_instruction=*/ false);
		  }
...
void EnterInterpreterFromDeoptimize(Thread* self,
  ...
  while (shadow_frame != nullptr) {
    ...
    if (UNLIKELY(self->IsExceptionPending())) 
      ...
      const instrumentation::Instrumentation* const instrumentation = frame_cnt == 0 ? nullptr : Runtime::Current()->GetInstrumentation();
      new_dex_pc = MoveToExceptionHandler(self, *shadow_frame, instrumentation) ? shadow_frame->GetDexPC() : dex::kDexNoIndex;
    } 
		else if (!from_code) 
		{
      // Deoptimization is not called from code directly.
      const Instruction* instr = &accessor.InstructionAt(dex_pc);
      if (deopt_method_type == DeoptimizationMethodType::kKeepDexPc || shadow_frame->GetForceRetryInstruction()) 
			{
