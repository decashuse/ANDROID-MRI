...
//decash custome function 
void ArtMethod::OriginalCode(Thread* self,  uint32_t* args, uint32_t args_size, JValue* result, const char* shorty) 
{
...
void ArtMethod::DisplayInputArgs(Thread* self, uint32_t* args, uint32_t args_size, const char* shorty) 
{
			// PRE  ////////////////////
			size_t arg_pos = 0;
			size_t arg_count = args_size/4;
			if (!IsStatic()) 
			{
					arg_pos++;
			}
		  ...
		  if( is_display_args() )
			  for (size_t shorty_pos = 0; arg_pos < arg_count; ++shorty_pos, ++arg_pos) 
			  {
				  switch (shorty[shorty_pos + 1]) 
				  {
					  case 'L': 
					  {
								  ObjPtr<mirror::Object> obj = reinterpret_cast<StackReference<mirror::Object>*>(&args[arg_pos])->AsMirrorPtr();
								  if ( obj == nullptr ) 
										  break;
								  ...
								  if(  obj->IsString() )
								  ...
								  else
									{
										  DECASH_ART("  └[" + std::to_string(shorty_pos+1) + "] " + "[CLASS]" + " => " + obj->PrettyTypeOf() );
											//obj->GetClass()->DecashDumpClass( DECASH_ART(""), mirror::Class::kDumpClassFullDetail);
											///////////////


											/*

											// DISP Fields(InstanceFields) Info
											for (ArtField& field : obj->GetClass()->GetIFields()) 
											{
															//DECASH_ART("     └[" + std::string(field.GetTypeDescriptor()) + " " +  field.GetName());
															DECASH_ART("     └[" + field.PrettyField() );
															//ObjPtr<mirror::Object> objField = field.GetDeclaringClass();
															//if(field.GetObject()->IsString())
															//  DECASH_ART("     └[" + field.PrettyField() + " => " + field.GetObject()->AsString()->ToModifiedUtf8() );

															//if( strcmp(field.GetTypeDescriptor(), "Ljava/lang/String;") == 0 )
															//{
															//  ObjPtr<mirror::Object> objx = obj->GetFieldObject<mirror::Object>(field.GetOffset()); 
															//  if(  objx->IsString() )
															//    //DECASH_ART("     └[" + field.PrettyField() + " => " + objx->AsString()->ToModifiedUtf8() );
															//    DECASH_ART("       └[ FUCK => " + objx->AsString()->ToModifiedUtf8() );
															//}
															//else
															//  DECASH_ART("     └[" + field.PrettyField() );
...
void ArtMethod::DisplayReturnArgs(JValue* result, const char* shorty) 
{
	if( is_display_args() )
			if( result != nullptr )
			{
...
void ArtMethod::DisplayInputSearchItems(Thread* self, uint32_t* args, uint32_t args_size, const char* shorty) 
{						
						size_t arg_pos = 0;
						size_t arg_count = args_size/4;
						if (!IsStatic()) 
						...
						for (size_t shorty_pos = 0; arg_pos < arg_count; ++shorty_pos, ++arg_pos) 
						{
								switch (shorty[shorty_pos + 1]) 
								{
										case 'L': 
										{
												...
												if(  obj->IsString() )
												{
																// check target search_item 
																if( check_target_search_item(obj->AsString()->ToModifiedUtf8().c_str()) )
																{	

																				//if( is_display_stack_trace() ) self->DecashDump(DECASH_ART(""), false);
																				if(self == NULL) { int xx = 1; xx++; }

																				DECASH_ART( "[N]" + PrettyMethod() );
																				DECASH_ART( "  └[" + std::to_string(shorty_pos+1) + "] " + "[CLASS.STRING]" + " => " + obj->AsString()->ToModifiedUtf8() );

																				//Thread* selfx = Thread::Current();
																				//if( is_display_stack_trace() ) selfx->DecashDump(DECASH_ART(""), false);
																}
												}
...
void ArtMethod::DisplayReturnSearchItems(Thread* self, JValue* result, const char* shorty) 
{
			// DISPALY Return
			if( result != nullptr )
			{
...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  {											
		  ...
		  if(getuid() > 10100 )
		  {
				  ...
				  if( !IsNative() && !IsProxyMethod() )
				  {


						  if (IsStatic()) 
						  {
								  art::interpreter::EnterInterpreterFromInvoke(self, this, nullptr, args, result, true);
						  }
						  else
						  {
								  mirror::Object* receiver = reinterpret_cast<StackReference<mirror::Object>*>(&args[0])->AsMirrorPtr();
								  art::interpreter::EnterInterpreterFromInvoke(self, this, receiver, args + 1, result, true);
						  }    
				  }    
				  else
				  {
						  if( is_target_uid() && (get_trace_mode() == 4) /*&& (strncmp(GetIsoDate().c_str(), "2023", 4) == 0)*/)
						  {
								  // SEARCH_ITEM mode
								  if( is_search_item_exist() )
								  {
								  		  if( check_target_method( PrettyMethod().c_str() ) )
										  {
										  		DisplayInputSearchItems(self, args, args_size, shorty);
										  		OriginalCode(self, args, args_size, result, shorty);
										  		DisplayReturnSearchItems(self, result, shorty);
										  }
										  else
										  		OriginalCode(self, args, args_size, result, shorty);
								  }
								  ...
								  // NORMAL HOOKA mode
								  else if( check_target_method( PrettyMethod().c_str() ) )
								  {
										  DisplayInputArgs(self, args, args_size, shorty); 
										  OriginalCode(self, args, args_size, result, shorty);
										  DisplayReturnArgs(result, shorty); 
								  }
